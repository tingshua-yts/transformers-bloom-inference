...
def replace_transformer_layer(orig_layer_impl, model, checkpoint_dict, config, model_config):
    ...
    def replace_with_policy(child, policy_cls, triangular_masking, inference=False, layer_id=0):
        logger.info("replace_with_policy")
    ...
    logger.info(f"AAAAAAAAA: checkpoint_dict:{checkpoint_dict}")
    ...
    logger.info(f"AAAAAAAAA config.save_mp_checkpoint_path: {config.save_mp_checkpoint_path}")
...
def replace_module(model, orig_class, replace_fn, _replace_policy):
    ...
    if orig_class is not None:
        logger.info(f"replace_module: {orig_class}")
        ...
        for plcy in replace_policies:
            ...
            if isinstance(plcy._orig_layer_class, list):
                for orig_layer_class in plcy._orig_layer_class:
                    logger.info(f"replace_module: {orig_layer_class}")
                    policy.update({orig_layer_class: (replace_fn, plcy)})
            elif plcy._orig_layer_class is not None:
                logger.info(f"replace_module: {plcy._orig_layer_class}")
                policy.update({plcy._orig_layer_class: (replace_fn, plcy)})
    assert len(policy.items()) > 0,\
        "No default policy found! Please specify your policy injection_policy (like {BertLayer:HFBEertLayerPolicy})." +\
        "You can find some samples here: https://github.com/microsoft/DeepSpeed/blob/master/deepspeed/module_inject/replace_policy.py"
    logger.info(f"model:{model}, orig_class:{orig_class}, policy:{policy}")
...
def _replace_module(model, policies, layer_id=0):
    ...
    for name, child in model.named_children():
        logger.info(f"name:{name}, child:{child}")
        if child.__class__ in policies:
            # policies为一个二元组：第一个为replace_fn; 第二个为BLOOMLayerPolicy
            replaced_module = policies[child.__class__][0](child, policies[child.__class__][-1], layer_id)
            logger.info(f"{child.__class__} in policies execute replace with {replaced_module}")
