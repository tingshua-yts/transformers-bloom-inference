...
def load_model_with_checkpoint(r_module,
                               ...
                               rank=0,
                               container=None):
    logger.info("load_model_with_checkpoint")
    error_msgs = []
    ...
    def load(module, prefix):
        logger.info(f"load_no_transformer_layer , module:{module.__class__}, prefix:{prefix}")
    ...
    def load_transformer_layer(module, prefix):
        logger.info(f"load_transformer_layer , module:{module.__class__}, prefix:{prefix}, ckpt_type:{ckpt_type}")
        if ckpt_type == "tp":
            logger.info(f"load_transformer_layer in tp")

            def load_parameters(module, prefix):
                for n, p in module.named_parameters():
                    if prefix + n in sd[0] and len(n.split('.')) == 1:
                        logger.info(f"load_parameters keys: {prefix + n}")
            ...
            logger.info(f"load_transformer_layer in pp")
    ...
    def load_module_recursive(module, prefix='', level=0):
        logger.info(f"load_module_recursive, module: {module.__class__}, prefix: {prefix}, level: {prefix}")
        for name, child in module.named_children():
            if child.__class__ in layer_policies:
                ...
                load_module_recursive(
                    ...
                    prefix + name + '.',
                    level + 1)
    logger.info("before load_module_recursive")
    load_module_recursive(r_module)
    logger.info("after load_module_recursive")
